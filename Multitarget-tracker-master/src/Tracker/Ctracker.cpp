...
#pragma omp parallel for
    ...
    if (!m_tracks.empty())
    {
        ...
        distMatrix_t costMatrix_pad(paddedSize * paddedSize, FLT_MAX);
        ...
        for (int i = 0; i < N; i++)						// i->row
        {
            ...
            {
                costMatrix_pad[i + j * paddedSize] = costMatrix[i + j * N];// 列扫描
            }
        }

        for (int i = 0; i < N; i++) // (i, M+i)
            costMatrix_pad[i + (M + i) * paddedSize] = 10; // costUnmatchedTracks
        for (int j = 0; j < M; j++)	// (N+j, j)
            costMatrix_pad[N + j + j * paddedSize] = 10;	// costUnmatchedDetecions
        for (int i = N; i < paddedSize; i++)
            for (int j = M; j < paddedSize; j++)
                costMatrix_pad[i + j * paddedSize] = 0;

        // Solving assignment problem (shortest paths)
        m_SPCalculator->Solve(costMatrix_pad, paddedSize, paddedSize, assignment_all, maxCost);
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            //std::cout << "size:%d" << assignment.size() << '\t' << "N:%d" << N << std::endl;
        }
