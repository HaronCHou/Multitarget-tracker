...
void MouseTracking(cv::CommandLineParser parser)
{
	...
	while (k != 27)
	{
		...
		regions_t regions;		// 把中心点pos封装为检测点的集合
		for (auto p : pts)
		{
		...
		{
#if (CV_VERSION_MAJOR >= 4)		// bgr 中心点为绿色
		...
		tracker->Update(regions, cv::UMat(), 100);		// fps=100，帧率太高，轨迹看起来就只有一条；要么就是度数加的太少
		...
		for (size_t i = 0; i < tracks.size(); i++)
		{
			...
			if (track.m_trace.size() > 1)
			{
				...
				{
#if (CV_VERSION_MAJOR >= 4)		// 只将最近两个轨迹连起来
